...
import { getCompanyByUserId, getWasteRecords } from "@/features/environmental/actions";
...
export default async function ResiduosPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50">
        <div className="text-center p-8 bg-white rounded-xl shadow-md border border-stone-200">
          ...
          <p className="text-stone-600 mb-6">Debes estar vinculado a una empresa para gestionar residuos.</p>
  ...
  const wasteRecords = await getWasteRecords(company.id);
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <WasteManagementClient initialData={JSON.parse(JSON.stringify(wasteRecords))} />
    </div>
  );
}